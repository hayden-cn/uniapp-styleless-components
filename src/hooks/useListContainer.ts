...
declare global {
  ...
  interface GlobalConfig {
    listContainerHook: ListContainerConfig<
      AnyObject,
      AnyObject,
      Pagination,
      ListResponse
   ...
   * 列表分页参数配置
   */
  type Pagination = GetValue<
    DefineListContainerType,
    "pagination",
    InnerPagination
  >;
   ...
   * 列表数据返回结构
   */
  type ListResponse<Data extends AnyObject = AnyObject> = GetValue<
    DefineListContainerType<Data>,
    "response",
...
> extends ListContainerConfig<Params, Data, Paging, Response> {
   ...
   * 滚动时回调
   */
  onScroll?: (e: PointerEvent) => void | Promise<void>;
  /**
   * 下拉刷新后回调
   */
  onRefresh?: (e?: PointerEvent) => void | Promise<void>;
  /**
   * 加载更多数据后回调
   */
  onLoadMore?: (e?: PointerEvent) => void | Promise<void>;
}
...
>(
  ...
  const queryParams = ref<Params>(cloneDeep(defaultQueryParams));
  ...
  const refresh = async (e?: PointerEvent) => {
    try {
      ...
      // 解析分页参数
      const paging = resolvePagination(cloneDeep(pagination.value));
      // 构建请求参数
      const requestParams = cloneDeep(queryParams.value);
      // 获取列表数据
  ...
  const loadMore = async (e?: PointerEvent) => {
    try {
  ...
  const onScroll = debounce((e: PointerEvent) => {
