  ...
  <view :key="item.field" :class="itemClassNames" :style="itemStyles">
    <view class="form-group-relative">
      ...
      <view :class="titleClassNames" :style="titleStyles">
        ...
        <view class="form-group-label-suffix">
          <slot
...
<script lang="ts">
type FormItemGroupProps<Data extends Record<string, any> = any> = {
  /**
   * 指定当前字段为一个字段分组
   */
  type: "group";
...
declare global {
   ...
   * 表单项分组
   */
  type FormItemGroup<Data extends Record<string, any> = any> = {
    /**
     * 分组字段名
     */
    field: string;
  } & FormItemGroupProps<Data> &
    BaseFormItem;
}
</script>
...
const labelClassNames = computed(() => {
  const classNames: ClassNameValue = ["form-group-label"];
